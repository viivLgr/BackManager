  ...
  <div class="container">
    ...
    <div class="operating">
        <el-form :inline="true" :model="searchForm" :rules="rules" ref="searchForm" class="demo-form-inline" size="mini">
            ...
            <el-form-item label="银行名称" prop="bankName">
                <el-input v-model="searchForm.bankName" placeholder="请输入银行名称(支持模糊查询)"></el-input>
            </el-form-item>
            <el-form-item>
    ...
    <div class="table">
      ...
      <el-table
        ...
        <el-table-column align="center" prop="supportBankTypeDesc" label="支持银行卡类型" width="120"></el-table-column>
        ...
        <el-form ref="form" :model="form" size="small" label-width="110px">
          ...
          <el-form-item label="支持银行卡类型">
              <el-select v-model="form.supportiveBankType" placeholder="请选择银行卡类型">
                  <el-option v-for="(item, index) in cardType" :key="index" :label="item.dicName" :value="item.dicCode"></el-option>
              </el-select>
          </el-form-item>
...
export default {
  ...
  methods: {
      ...
      _router.getSupportBankList(data).then(res => {
        _this.renderTableDate(res, (item, index) => {
          return {
            ...
            supportBankTypeDesc: item.supportBankTypeDesc,
    ...
    handleForm(row) {
      const _this = this;
      if (!_this.cardType.length) {
        this.getCardType();
      }
      if (!_this.bankList.length) {
        this.getBinkList();
      }
      if (row instanceof Object) {
        ...
        _this.form = {
          title: "添加",
          channelId: _this.searchForm.channelId,
          channelName: _this.searchForm.channelName
        };
      }
      _this.addShow = true;
    },
    ...
    formSubmit() {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          delete _this.form.channelName;
          if (_this.form.title === "添加") {
            delete _this.form.title;
            _router.addSupportBankList(_this.form).then(res => {
              _this.formatResult(res, "添加成功");
            });
          } else {
            delete _this.form.title;
            delete _this.form.channelId;
            _router.updateSupportBankList(_this.form).then(res => {
    ...
    formatResult(res, msg) {
      ...
      _this.filterAxios(res, res => {
        ...
        this._renderTableDate({ channelId: _this.searchForm.channelId });
      });
    }
  }
};
