          ...
          <template v-if="item.prop === 'todo'">
            ...
            >
              <template slot-scope="scope">
                <el-button @click.stop="pass(scope.row)" type="text" style="color: #3ac100;" size="small">通过</el-button>
                <el-button @click.stop="deny(scope.row)" type="text" style="color: #ff3232;" size="small">拒绝</el-button>
              </template>
            </el-table-column>
          </template>
          <template v-else>
...
//            {required: true, message: '请输入商户编号', trigger: 'blur'},
    ...
    methods: {
      pass (row) {
        const {storeId, storeName, shortName} = row
        this.$confirm(`您将通过${storeName}(${shortName})的审核, 是否继续?`, '提示', {
        ...
        console.log('pass', row, storeId)
      },
      deny (row) {
        const {storeId, storeName, shortName} = row
        this.$confirm(`您将拒绝${storeName}(${shortName})的审核, 是否继续?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
